...
describe('local date utilities', () => {
  it('formats local components without UTC conversion', () =>
    expect(localDateKey(new Date(2026, 6, 18, 0, 5))).toBe('2026-07-18'));
  it('accepts leap day', () =>
  ...
  it('formats the same local calendar date in Vietnamese and English', () => {
    ...
    expect(formatLongLocalDate('2026-01-02', 'en')).toBe(
  ...
  it('rejects an invalid local date before formatting', () =>
    expect(() => formatLongLocalDate('2026-02-31', 'en')).toThrow(
      'Invalid local date',
    ));
});
