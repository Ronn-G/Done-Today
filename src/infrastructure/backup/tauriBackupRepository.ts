...
const previewSchema=z.object({fileName:z.string(),format:z.literal('done-today-backup'),version:z.literal(1),exportedAt:z.string(),
  ...
  previouslyImportedAt:z.string().nullable(),warnings:z.array(z.unknown())})
  .transform(value=>({...value,warnings:value.warnings.map(normalizeAppWarning)}));
...
export class TauriBackupRepository implements BackupRepository{
  ...
  async export(path:string):Promise<ExportResult>{return exportSchema.parse(await invokeWithAppError(this.invokeCommand,'export_backup',{path}))}
  async preview(path:string):Promise<ImportPreview>{return previewSchema.parse(await invokeWithAppError(this.invokeCommand,'preview_backup',{path}))}
  async import(path:string,mode:ImportMode,applyTheme:boolean,confirmReimport:boolean):Promise<ImportResult>{
    return importSchema.parse(await invokeWithAppError(this.invokeCommand,'import_backup',{path,mode,applyTheme,confirmReimport}));
  }
}
