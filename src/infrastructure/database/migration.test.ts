...
function seed(db: InstanceType<SqlJs['Database']>) {
  db.run('INSERT INTO daily_logs VALUES (?,?,?,?)', [
    'log',
    '2026-07-18',
    'now',
    'now',
  ]);
  db.run('INSERT INTO work_items VALUES (?,?,?,?,?,?,?,?,?)', [
    'item',
    'log',
    'Task',
    '',
...
describe('migration v1', () => {
  it('creates the required tables', () => {
    ...
    const rows = db
      ...
      .values.flat();
    expect(rows).toEqual(
      expect.arrayContaining(['daily_logs', 'schema_migrations', 'work_items']),
    );
  });
  ...
  it('rejects an invalid status', () => {
    ...
    expect(() =>
      db.run(
        "INSERT INTO work_items VALUES ('i','log','Task','','','invalid',0,'now','now')",
      ),
    ).toThrow();
  });
  it('enforces foreign keys and cascade delete', () => {
    ...
    expect(() =>
      db.run(
        "INSERT INTO work_items VALUES ('i','missing','Task','','','completed',0,'now','now')",
      ),
    ).toThrow();
    seed(db);
    db.run("DELETE FROM daily_logs WHERE id='log'");
