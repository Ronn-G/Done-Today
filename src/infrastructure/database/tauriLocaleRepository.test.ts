...
describe('TauriLocaleRepository',()=>{
  it('uses the WebView preferred locale in one typed initialization command',async()=>{
    const invoke=vi.fn(async(command:string)=>command==='initialize_locale_preference'
      ?{locale:'en',source:'fresh'}:undefined);
    const repository=new TauriLocaleRepository(invoke,()=> 'en-US');
    await expect(repository.initialize()).resolves.toEqual({locale:'en',source:'fresh'});
  ...
  it('uses a null locale when the WebView locale API is unavailable',async()=>{
    ...
    const repository=new TauriLocaleRepository(invoke,()=>{throw new Error('unavailable')});
    await expect(repository.initialize()).resolves.toEqual({locale:'en',source:'fresh'});
    expect(invoke).toHaveBeenCalledWith('initialize_locale_preference',{osLocale:null});
  });
  it('normalizes initialization infrastructure errors',async()=>{
    const failure=new Error('invoke failed');const invoke=vi.fn(async(command:string)=>{if(command==='initialize_locale_preference')throw failure});
    const repository=new TauriLocaleRepository(invoke,()=> 'vi-VN');
    await expect(repository.initialize()).rejects.toEqual({kind:'unknown'});
    expect(invoke).not.toHaveBeenCalledWith('save_locale_preference',expect.anything());
  });
  ...
  it('rejects a malformed native bootstrap response',async()=>{
    const repository=new TauriLocaleRepository(async()=>({locale:'fr',source:'fresh'}),()=> 'fr-FR');
