...
export class TauriLocaleRepository implements LocaleRepository{
  ...
  ){this.invokeCommand=invokeCommand;this.readPreferredLocale=readPreferredLocale}
  private async ensureDatabase(){if(!this.initialized){await invokeWithAppError(this.invokeCommand,'initialize_database');this.initialized=true}}
  async initialize(){
    let osLocale:unknown=null;
    try{osLocale=this.readPreferredLocale()}catch{osLocale=null}
    const result=localeInitializationSchema.parse(
      await invokeWithAppError(this.invokeCommand,'initialize_locale_preference',{
  ...
  async save(locale:AppLocale){await this.ensureDatabase();await invokeWithAppError(this.invokeCommand,'save_locale_preference',{locale})}
}
