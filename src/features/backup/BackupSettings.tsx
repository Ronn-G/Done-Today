...
export type BackupResult =
  | { kind: 'export'; value: ExportResult }
  | { kind: 'import'; value: ImportResult };

export function backupDialogPresentation(t: TFunction<'backup'>): {
  export: BackupDialogPresentation;
  import: BackupDialogPresentation;
} {
  ...
  t('export.success', {
    ...
    summary: formatList(
      [
        ...
        t('export.summary.workCategories', {
          count: value.counts.workCategories,
        }),
        value.counts.theme
          ? t('export.summary.themeIncluded')
          : t('export.summary.themeExcluded'),
      ],
      locale,
    ),
  });
const importSummary = (
  ...
  if (value.remapped > 0)
    parts.push(t('import.summary.remapped', { count: value.remapped }));
...
export function BackupSettings({ flushTheme, onImported }: Props) {
  ...
  const service = useMemo(
    () =>
      new BackupService(new TauriBackupRepository(), flushTheme, onImported),
    [flushTheme, onImported],
  );
  const [state, setState] = useState<BackupOperation>('idle');
  ...
  const runExport = async () => {
    ...
    try {
      setState('choosing');
      const value = await service.export(backupDialogPresentation(t).export);
      if (!value) {
        setState('idle');
        return;
      }
      setResult({ kind: 'export', value });
      setState('success');
    } catch (reason) {
  ...
  const chooseImport = async () => {
    ...
    try {
      setState('validating');
      const value = await service.chooseAndPreview(
        backupDialogPresentation(t).import,
      );
      if (!value) {
        setState('idle');
        return;
  ...
  const runImport = async () => {
    ...
    setError(null);
    try {
      const value = await service.import(
        selected.path,
        mode,
        applyTheme,
        reimportConfirmed,
    ...
    <>
      ...
      {selected && (
        <ImportPreviewDialog
          ...
          setReplaceConfirmed={setReplaceConfirmed}
          reimportConfirmed={reimportConfirmed}
          setReimportConfirmed={setReimportConfirmed}
          busy={state === 'importing'}
          error={error}
          onCloseError={closeError}
          cancel={() => setSelected(null)}
          submit={() => void runImport()}
        />
      )}
    </>
  );
...
export function BackupSettingsView({
  ...
  const titleId = useId();
  const busy = ['preparing', 'choosing', 'validating', 'importing'].includes(
    state,
  );
  const resultText = result
    ? result.kind === 'export'
      ? exportSummary(result.value, t, locale)
      : importSummary(result.value, t, locale)
    : null;
  return (
    <section
    ...
    >
      ...
      <div className="backup-actions">
        <button type="button" disabled={busy} onClick={onExport}>
          <Download aria-hidden="true" size={17} /> {t('export.action')}
        </button>
        <button type="button" disabled={busy} onClick={onChooseImport}>
          <Upload aria-hidden="true" size={17} /> {t('import.action')}
        </button>
        {busy && (
      ...
      {resultText && (
        <p className="backup-success" role="status" aria-live="polite">
          {resultText}
        </p>
      )}
      {error != null && (
        <div className="page-error" role="alert">
          <span>{backupErrorMessage(error, tErrors)}</span>
          <button type="button" onClick={onCloseError}>
            {String(tCommon('actions.close'))}
          </button>
        </div>
      )}
    </section>
...
export function ImportPreviewDialog({
  ...
  applyTheme: boolean;
  setApplyTheme: (value: boolean) => void;
  replaceConfirmed: boolean;
  setReplaceConfirmed: (value: boolean) => void;
  reimportConfirmed: boolean;
  setReimportConfirmed: (value: boolean) => void;
  busy: boolean;
  error: NormalizedAppError | null;
  onCloseError: () => void;
  cancel: () => void;
  ...
  const titleId = useId(),
    fileId = useId(),
    mergeId = useId(),
    mergeDescriptionId = useId(),
    replaceId = useId(),
    replaceDescriptionId = useId();
  const replaceConfirmId = useId(),
    reimportConfirmId = useId(),
    applyThemeId = useId();
  const dialogRef = useRef<HTMLElement>(null),
    initialFocusRef = useRef<HTMLButtonElement>(null);
  useEffect(() => {
    const returnFocus =
  ...
  const allowed =
    (mode === 'merge' || replaceConfirmed) &&
    (!value.previouslyImportedAt || reimportConfirmed);
  const dataSummary = formatList(
    [
      ...
      value.counts.theme
        ? t('preview.data.themeIncluded')
        : t('preview.data.themeExcluded'),
    ],
    locale,
  );
  ...
  return (
    <div className="dialog-backdrop" role="presentation">
      ...
      >
        <h2 id={titleId}>{t('preview.title')}</h2>
        <p id={fileId}>
          <strong>{value.fileName}</strong>
        </p>
        <dl>
          <div>
            <dt>{t('preview.metadata.format')}</dt>
            <dd>
              {value.format} v{value.version}
            </dd>
          </div>
          <div>
            <dt>{t('preview.metadata.exportedAt')}</dt>
            <dd>{formatDateTime(new Date(value.exportedAt), locale)}</dd>
          </div>
          <div>
            <dt>{t('preview.metadata.appVersion')}</dt>
            <dd>{value.appVersion}</dd>
          </div>
          <div>
            <dt>{t('preview.metadata.checksum')}</dt>
            <dd>
              {value.checksumValid
                ? t('preview.checksum.valid')
                : t('preview.checksum.invalid')}
            </dd>
          </div>
          <div>
            <dt>{t('preview.metadata.data')}</dt>
            <dd>{dataSummary}</dd>
          </div>
          <div>
            <dt>{t('preview.metadata.dryRun')}</dt>
            <dd>{dryRunSummary}</dd>
          </div>
        </dl>
        {value.warnings.map((warning, index) => (
          <p
            ...
            key={`${warning.kind === 'known' ? warning.code : 'unknown'}-${index}`}
          >
            {localizeAppWarning(warning, toErrorTranslator(tErrors))}
          </p>
        ))}
        {error != null && (
          <div className="page-error" role="alert">
        ...
        {value.previouslyImportedAt && (
          <label className="confirm-row" htmlFor={reimportConfirmId}>
            <input
              id={reimportConfirmId}
              type="checkbox"
              checked={reimportConfirmed}
              onChange={(event) => setReimportConfirmed(event.target.checked)}
            />
            <span>
              {t('confirm.reimport', {
                dateTime: formatDateTime(
                  new Date(value.previouslyImportedAt),
                  locale,
                ),
              })}
            </span>
        ...
        <fieldset>
          ...
          <label htmlFor={mergeId}>
            <input
              ...
              value="merge"
              checked={mode === 'merge'}
              aria-describedby={mergeDescriptionId}
              onChange={() => setMode('merge')}
            />
            <span>
              <strong>{t('mode.merge.label')}</strong>
              <span id={mergeDescriptionId} className="choice-description">
                {t('mode.merge.description')}
              </span>
            </span>
          </label>
          <label className="danger-choice" htmlFor={replaceId}>
            <input
              id={replaceId}
              name="backup-import-mode"
              type="radio"
            ...
            />
            <span>
              <strong>{t('mode.replace.label')}</strong>
              <span id={replaceDescriptionId} className="choice-description">
                {t('mode.replace.description')}
              </span>
            </span>
          </label>
        </fieldset>
        {mode === 'merge' && value.counts.theme && (
          <label className="confirm-row" htmlFor={applyThemeId}>
            <input
              id={applyThemeId}
              type="checkbox"
              checked={applyTheme}
            ...
            />
            <span>{t('options.applyTheme')}</span>
          </label>
        )}
        {mode === 'replace' && (
          <label className="confirm-row danger-box" htmlFor={replaceConfirmId}>
            <input
              id={replaceConfirmId}
              type="checkbox"
            ...
            />
            <span>
              <strong>{t('confirm.replace.title')}</strong>
              <span className="choice-description">
                {t('confirm.replace.body')}
              </span>
            </span>
          </label>
        )}
