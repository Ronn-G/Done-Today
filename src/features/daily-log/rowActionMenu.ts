...
export function getRowActionDestinations(categories: readonly WorkCategory[]) {
  return categories.filter((category) => category.isActive);
}
export function positionRowActionMenu(
  trigger: Pick<DOMRect, 'left' | 'right' | 'top' | 'bottom'>,
  viewport: { width: number; height: number },
  menu = { width: 224, height: 260 },
  margin = 8,
) {
  const left = Math.max(
    margin,
...
export async function moveItemAfterFlush(
  categoryId: string | null,
  flush: () => Promise<void>,
  move: (id: string | null) => Promise<void>,
) {
  await flush();
  await move(categoryId);
}
