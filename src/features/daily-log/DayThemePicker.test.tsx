...
function props(
  overrides: Partial<DayThemePickerProps> = {},
): DayThemePickerProps {
  return {
...
describe('DayThemePicker', () => {
  it.each([
    [
      'vi',
      'Chủ đề của ngày',
      ['Done Today Mặc định', 'Sakura', 'Coffee', 'Rainy'],
    ],
    ['en', 'Day theme', ['Done Today Default', 'Sakura', 'Coffee', 'Rainy']],
  ] as const)(
    'renders curated localized choices in %s',
    async (locale, title, names) => {
      ...
      names.forEach((name, index) =>
        expect(choices[index].textContent).toContain(name),
      );
      expect(choices[1].getAttribute('aria-checked')).toBe('true');
      expect(
        document.querySelectorAll('.day-theme-picker-thumbnail'),
      ).toHaveLength(4);
  ...
  it('keeps preview separate from persistence and restores it on cancel, Escape and outside close', async () => {
    ...
    fireEvent.mouseDown(
      document.querySelector('.day-theme-picker-backdrop') as HTMLElement,
    );
  ...
  it('supports arrow selection, default normalization and keyboard apply', async () => {
    ...
    expect(value.onApplied).toHaveBeenCalledWith({
      themeId: null,
      themeVersion: null,
    });
  });
  ...
  it('uses the safe runtime fallback for unknown persisted metadata without writing it', async () => {
    await initializeI18n('en');
    const value = props({
      persisted: { themeId: 'future-theme', themeVersion: 7 },
    });
    render(<DayThemePicker {...value} />);
    expect(
      screen
        .getByRole('radio', { name: /Done Today Default/ })
        .getAttribute('aria-checked'),
    ).toBe('true');
    expect(value.onApply).not.toHaveBeenCalled();
  });
