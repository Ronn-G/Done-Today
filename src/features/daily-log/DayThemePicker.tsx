...
function runtimeSelectionId(persisted: DayThemeMetadata): string {
  return dayThemeRegistry.resolve(persisted.themeId, persisted.themeVersion)
    .definition.id;
}

function translatedThemeCopy(
  id: string,
  t: TFunction<'theme'>,
  ...
  switch (id) {
    case 'sakura':
      return {
        name: t('dayTheme.sakura.name'),
        description: t('dayTheme.sakura.description'),
      };
    case 'coffee':
      return {
        name: t('dayTheme.coffee.name'),
        description: t('dayTheme.coffee.description'),
      };
    case 'rainy':
      return {
        name: t('dayTheme.rainy.name'),
        description: t('dayTheme.rainy.description'),
      };
    default:
      return {
...
export function DayThemePicker({
  ...
  useEffect(() => {
    const returnFocus =
      document.activeElement instanceof HTMLElement
        ...
        : null;
    itemRefs.current[
      definitions.findIndex((theme) => theme.id === selectedId)
    ]?.focus();
    return () => {
  ...
  const defaultDefinition = definitions.find(
    (theme) => theme.id === DEFAULT_DAY_THEME_ID,
  );

  return (
    <div
      className="dialog-backdrop day-theme-picker-backdrop"
      role="presentation"
    ...
    >
      ...
      >
        <header className="day-theme-picker-heading">
          <div>
            <h2 id={titleId}>{t('dayTheme.picker.title')}</h2>
            <p id={descriptionId}>{t('dayTheme.picker.description')}</p>
          </div>
          <button
            type="button"
            className="day-theme-picker-close"
            aria-label={t('dayTheme.picker.close')}
            disabled={saving}
            onClick={cancel}
          >
        ...
        <div
          className="day-theme-picker-grid"
          role="radiogroup"
          aria-label={t('dayTheme.picker.optionsLabel')}
        >
          {definitions.map((definition, index) => {
            ...
            return (
              ...
              >
                <span
                  className="day-theme-picker-thumbnail"
                  aria-hidden="true"
                  style={{ background: definition.cover.fallbackGradient }}
                />
                  ...
                  <span className="day-theme-picker-item-state">
                    {isCurrent && (
                      <span>
                        ...
                        {t('dayTheme.picker.current')}
                      </span>
                    )}
                    {isDraft && !isCurrent && (
                      <span>{t('dayTheme.picker.preview')}</span>
                    )}
                  </span>
                </span>
              </button>
        ...
        <div className="day-theme-picker-actions">
          <button
            ...
            disabled={saving || !defaultDefinition}
            onClick={() => defaultDefinition && selectDraft(defaultDefinition)}
          >
            <RotateCcw aria-hidden="true" size={16} />{' '}
            {t('dayTheme.picker.useDefault')}
          </button>
