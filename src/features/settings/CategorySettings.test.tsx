...
import {
  ...
  type CategorySettingsError,
  type CategorySettingsViewProps,
} from './CategorySettings';

const category: WorkCategory = {
  id: 'category-42',
  name: 'Công việc của Lan',
  color: '#4F7CAC',
  position: 3,
  isActive: true,
...
const renderView = (overrides: Partial<CategorySettingsViewProps> = {}) => {
  ...
  const props: CategorySettingsViewProps = {
    ...
    loading: false,
    error: null,
    name: 'Bản nháp giữ nguyên',
    color: '#112233',
    ...actions,
    ...overrides,
  };
  return {
    html: renderToStaticMarkup(<CategorySettingsView {...props} />),
    actions,
    props,
  };
};
...
describe('CategorySettings localization', () => {
  it('renders the complete existing Vietnamese workflow with accessible names', async () => {
    ...
    expect(html).toContain('Bản nháp giữ nguyên');
  ...
  it('switches system copy to English while preserving persisted data and draft without side effects', async () => {
    ...
    expect(after.html).toContain(
      'aria-label="Hide category Công việc của Lan"',
    );
    for (const value of [
      category.name,
      category.color,
      ...
      expect(before.html).toContain(value);
      expect(after.html).toContain(value);
    }
    ...
    expect(
      Object.values(after.actions).every(
        (action) => action.mock.calls.length === 0,
      ),
    ).toBe(true);
    expect(after.html).not.toContain('categories.');
    expect(after.html).not.toContain('settings.');
  });
  ...
  it.each([
    ...
    'renders the safe localized %s error and retry action',
    async (error, copy) => {
      await initializeI18n('en');
      const { html } = renderView({ error: error as CategorySettingsError });
      ...
      if (
        ...
        expect(html).toContain('aria-describedby="category-settings-error"');
      for (const unsafe of [
        'SQLITE_CONSTRAINT',
        'C:\\Users\\long',
        'SELECT *',
  ...
  it('announces the localized loading state without fetching during render', async () => {
    await initializeI18n('vi');
    const { html, actions } = renderView({ loading: true, categories: [] });
    expect(html).toContain('role="status"');
    expect(html).toContain('aria-live="polite"');
    expect(html).toContain('Đang tải nhóm…');
    expect(
      Object.values(actions).every((action) => action.mock.calls.length === 0),
    ).toBe(true);
  });
});
...
describe('CategorySettings behavior helpers', () => {
  ...
  it('keeps stable category data and sends the exact create/update/reactivation payloads', async () => {
    const created = { ...category, id: 'created' };
    const updated = { ...category, name: 'Tên mới' };
    const restored = { ...category, isActive: true };
    const service = {
      ...
      updateCategory: vi.fn(
        ...
        ) => (input.isActive ? updated : restored),
      ),
    };
    await expect(createCategory(service, 'Tên mới', '#AABBCC')).resolves.toBe(
      created,
    );
    ...
    expect(service.updateCategory).toHaveBeenLastCalledWith('category-42', {
      name: 'Tên mới',
      color: '#4F7CAC',
      isActive: true,
    });
    ...
    const latestDraft = {
      ...category,
      name: 'Bản nháp mới nhất sau lỗi',
      color: '#778899',
    };
  ...
  it('reorders with every stable ID and never creates a virtual uncategorized record', async () => {
    ...
    expect(result?.optimistic.map((value) => value.id)).toEqual([
      'category-99',
      'category-42',
    ]);
