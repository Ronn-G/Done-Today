...
const makeController = (
  preferences: ThemePreferences = defaultThemePreferences(),
  overrides: Partial<ThemeCustomizerController> = {},
...
const panelState: FloatingThemePanelState = {
  x: 120,
  y: 88,
  collapsed: true,
  open: true,
...
describe('I18N-3 Theme modes and presets', () => {
  it.each([
    ...
    'renders localized %s mode controls with technical selection unchanged',
    async (locale, heading, light, dark, system) => {
      await initializeI18n(locale);
      const onSelect = vi.fn();
      const html = renderToStaticMarkup(
        <ThemeModeSettings mode="system" onSelect={onSelect} />,
      );
  ...
  it.each([
    ...
    [
      'en',
      ['Done Today', 'Forest', 'Ocean', 'Lavender', 'Warm Sand', 'Monochrome'],
    ],
  ] as const)(
    'renders all localized %s preset metadata and decorative swatches',
    async (locale, names) => {
      await initializeI18n(locale);
      const onSelect = vi.fn();
      const preferences = selectPreset('ocean');
      const before = structuredClone(preferences);
      const html = renderToStaticMarkup(
        <ThemePresetSettings preferences={preferences} onSelect={onSelect} />,
      );
      for (const name of names)
        expect(html).toContain(`<strong>${name}</strong>`);
      expect(html).toContain('aria-hidden="true"');
      expect(html.match(/aria-pressed="true"/g)).toHaveLength(1);
      expect(html).not.toContain('theme.preset');
      expect(onSelect).not.toHaveBeenCalled();
  ...
  it('switches visible locale without a repository callback or technical-state change', async () => {
    ...
    const vietnamese = renderToStaticMarkup(
      <>
        <ThemeModeSettings mode="dark" onSelect={onMode} />
        <ThemePresetSettings preferences={preferences} onSelect={onPreset} />
      </>,
    );
    ...
    expect(vietnamese).toContain('Chế độ hiển thị');
    expect(english).toContain('Display mode');
    expect(vietnamese).toContain('Rừng xanh');
    expect(english).toContain('Forest');
...
describe('I18N-3 Custom colors and floating customizer', () => {
  it('maps exactly all 33 ThemeColorKey values to resolvable semantic translation keys', async () => {
    ...
    for (const locale of ['vi', 'en'] as const) {
      ...
      for (const [colorKey, translationKey] of Object.entries(
        themeColorTranslationKeys,
      )) {
      ...
      expect(themeColorTranslationKeys.tableHeaderBackground).toBe(
        'colors.tableHeaderBackground',
      );
      expect(catalog[themeColorTranslationKeys.tableHeaderBackground]).toBe(
        locale === 'vi' ? 'Tiêu đề bảng' : 'Table header',
      );
    }
  });
  ...
  it.each([
    ...
    [
      ...
      'Choose color: App background',
      'HEX code: App background',
      'Corner radius',
      'All changes save automatically',
    ],
  ] as const)(
    'renders all localized %s color controls and accessible names',
    async (locale, heading, group, colorPicker, hexCode, radius, status) => {
      await initializeI18n(locale);
      ...
      expect(html).toContain(`aria-label="${colorPicker}"`);
      expect(html).toContain(`aria-label="${hexCode}"`);
      expect(html).toContain(radius);
      expect(html).toContain(status);
      expect(html.match(/type="color"/g)).toHaveLength(33);
      for (const colorKey of Object.keys(themeColorTranslationKeys))
        expect(html).not.toContain(`>${colorKey}<`);
      expect(html).not.toMatch(
        /theme\.(colors|groups|customize|radius|status)/,
      );
    },
  );

  it('normalizes valid HEX drafts and never promotes invalid input to theme state', () => {
    expect(resolveColorControlInput('#123456')).toEqual({
      draft: null,
      normalized: '#123456',
      invalid: false,
    });
    expect(resolveColorControlInput('#a3F')).toEqual({
      draft: '#a3F',
      normalized: null,
      invalid: false,
    });
    expect(resolveColorControlInput('#a3F', true)).toEqual({
      draft: null,
      normalized: '#AA33FF',
      invalid: false,
    });
    expect(resolveColorControlInput('#12')).toEqual({
      draft: '#12',
      normalized: null,
      invalid: false,
    });
    expect(resolveColorControlInput('#12', true)).toEqual({
      draft: null,
      normalized: null,
      invalid: true,
    });
    expect(resolveColorControlInput('not-a-color')).toEqual({
      draft: 'not-a-color',
      normalized: null,
      invalid: true,
    });
  ...
  it('keeps the table-header picker, HEX field, draft palette and live preview synchronized', async () => {
    ...
    const preferences = updateThemeColor(
      initial,
      'light',
      'tableHeaderBackground',
      pickerChange.normalized!,
    );
    ...
    const html = renderToStaticMarkup(
      <ThemeCustomizerContent controller={makeController(preferences)} />,
    );
    expect(preferences.selectedPresetId).toBe('custom');
    expect(preferences.lightColors.tableHeaderBackground).toBe('#123456');
    expect(preferences.darkColors.tableHeaderBackground).toBe(
      initial.darkColors.tableHeaderBackground,
    );
    expect(values.get('--bg-table-header')).toBe('#123456');
    expect(html).toMatch(
      /type="color" aria-label="Chọn màu: Tiêu đề bảng" value="#123456"/,
    );
  ...
  it.each([
    ...
    'translates %s contrast warnings without exposing internal field keys',
    async (locale, message, pair) => {
      await initializeI18n(locale);
      const preferences = updateThemeColor(
        defaultThemePreferences(),
        'light',
  ...
  it.each([
    ...
    'renders the localized %s custom-preset state without changing preset behavior',
    async (locale, name, description) => {
      await initializeI18n(locale);
      const preferences = updateThemeColor(
        selectPreset('forest'),
        'dark',
  ...
  it.each([
    [
      'vi',
      'Bảng tùy chỉnh giao diện',
      'Kéo bảng tùy chỉnh giao diện',
      'Đặt lại vị trí bảng tùy chỉnh',
      'Mở rộng bảng tùy chỉnh giao diện',
      'Đóng bảng tùy chỉnh giao diện',
      'Mở rộng',
      'Đóng',
    ],
    [
      ...
      'Drag the appearance customizer',
      'Reset the appearance customizer position',
      'Expand the appearance customizer',
      'Close the appearance customizer',
      'Expand',
      'Close',
    ],
  ] as const)(
    'renders localized %s floating-panel visible, tooltip, and accessibility copy',
    async (
      locale,
      label,
      drag,
      ...
      expect(html).toContain(`aria-label="${drag}"`);
      expect(html).toContain(`aria-label="${reset}"`);
      expect(html).toContain(`aria-label="${expand}"`);
      expect(html).toContain(`aria-label="${close}"`);
      expect(html).toContain(`title="${expandTitle}"`);
      expect(html).toContain(`title="${closeTitle}"`);
  ...
  it('switches locale without mutating theme or floating-panel technical state', async () => {
    ...
    const controller = makeController(preferences, {
      mode: 'dark',
      activePalette: 'dark',
    });
    const beforePreferences = structuredClone(preferences);
    const beforePanel = structuredClone(panelState);
    await initializeI18n('vi');
    const vietnamese = renderToStaticMarkup(
      <>
        <ThemeCustomizerContent controller={controller} />
        <FloatingThemeCustomizer
    ...
    expect(vietnamese).toContain('Tùy chỉnh màu');
    expect(english).toContain('Custom colors');
    expect(preferences).toEqual(beforePreferences);
    expect(panelState).toEqual(beforePanel);
    expect(controller.mode).toBe('dark');
    expect(controller.activePalette).toBe('dark');
    expect(controller.commit).not.toHaveBeenCalled();
    expect(controller.setMode).not.toHaveBeenCalled();
  ...
  it.each([
    ...
    'localizes %s save states and retry while retaining the existing coordinator callbacks',
    async (locale, saving, saved, error, retry) => {
      await initializeI18n(locale);
      const preferences = defaultThemePreferences();
      const savingHtml = renderToStaticMarkup(
        <ThemeCustomizerContent
          controller={makeController(preferences, { saveState: 'saving' })}
        />,
      );
      const savedHtml = renderToStaticMarkup(
        <ThemeCustomizerContent
          controller={makeController(preferences, { saveState: 'saved' })}
        />,
      );
      const retryCallback = vi.fn(async () => undefined);
      const errorHtml = renderToStaticMarkup(
        <ThemeCustomizerContent
          controller={makeController(preferences, {
            saveState: 'error',
            error: { kind: 'known', code: 'theme.invalid', params: {} },
            retry: retryCallback,
          })}
        />,
      );
