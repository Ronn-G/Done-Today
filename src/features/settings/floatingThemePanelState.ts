export const FLOATING_THEME_PANEL_STORAGE_KEY =
...
export function getDefaultPanelPosition(
  ...
  panel: PanelSize = {
    width: FLOATING_THEME_PANEL_WIDTH,
    height: FLOATING_THEME_PANEL_HEADER_HEIGHT,
  },
): PanelPosition {
  return clampPanelPosition(
    { x: viewport.width - panel.width - FLOATING_THEME_PANEL_MARGIN, y: 88 },
    viewport,
    panel,
  );
}
...
export function clampPanelPosition(
  ...
  const maxX = Math.max(
    FLOATING_THEME_PANEL_MARGIN,
    viewport.width - panel.width - FLOATING_THEME_PANEL_MARGIN,
  );
  const maxY = Math.max(
    FLOATING_THEME_PANEL_MARGIN,
    viewport.height - panel.height - FLOATING_THEME_PANEL_MARGIN,
  );
  return {
...
export function parsePanelState(
  ...
  try {
    ...
    if (
      candidate.schemaVersion !== FLOATING_THEME_PANEL_SCHEMA_VERSION ||
      !finite(candidate.x) ||
      !finite(candidate.y) ||
...
export function isInteractiveDragTarget(target: EventTarget | null): boolean {
  return Boolean(
    ...
    (
      target as unknown as { closest: (selector: string) => Element | null }
    ).closest(
...
export async function closeFloatingThemePanel(
  flush: () => Promise<void>,
  close: () => void,
): Promise<void> {
  try {
