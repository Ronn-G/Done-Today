...
} from '../../application/localization/localeSwitchCoordinator';
...
export function LanguageSettingsView({
  ...
  const statusMessage =
    state === 'saving'
      ? t('language.status.saving')
      : state === 'saved'
        ? t('language.status.saved')
        : state === 'error'
          ? t('language.status.error')
          : null;
  return (
    <section className="settings-card">
      ...
      <label>
        ...
        >
          <option value="vi">{t('language.option.vi')}</option>
...
export function LanguageSettings({
  service = defaultLocaleService,
  activate = changeAppLanguage,
}: {
  service?: Pick<LocaleService, 'save'>;
  ...
  const coordinator = useMemo(
    () =>
      new LocaleSwitchCoordinator(
        activate,
        (locale) => service.save(locale),
        setState,
      ),
    [activate, service],
  );
  ...
  const current = isAppLocale(i18n.resolvedLanguage)
  ...
  return (
    <LanguageSettingsView
      ...
      state={state}
      onChange={(locale) => {
        void coordinator.switchTo(locale).catch(() => undefined);
      }}
      onRetry={() => {
