...
export const formatNumber = (value: number, locale: AppLocale) =>
  numberFormatter(locale, { maximumFractionDigits: 3 }).format(finite(value));
...
) => dateFormatter(locale, options).format(validDate(value));
...
export const formatList = (values: readonly string[], locale: AppLocale) => {
  ...
  if (!formatter) {
    formatter = new Intl.ListFormat(intlLocale[locale], {
      style: 'long',
      type: 'conjunction',
    });
...
export const formatCount = (
  ...
  return {
    value: numberFormatter(locale, { maximumFractionDigits: 0 }).format(count),
    pluralCategory: rules.select(count),
  };
};
