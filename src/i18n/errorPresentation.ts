import {
  normalizeAppError,
  normalizeAppWarning,
} from '../application/errors/errorNormalizer';
import type {
  AppErrorCode,
  AppErrorParams,
  AppWarningCode,
...
export const appErrorPresentation = {
  ...
  'work_item.task_too_long': {
    key: 'today:backendErrors.taskTooLong',
    required: ['max'],
    paramTypes: { max: 'number' },
  },
  'work_item.result_too_long': {
    key: 'today:backendErrors.resultTooLong',
    required: ['max'],
    paramTypes: { max: 'number' },
  },
  'work_item.next_action_too_long': {
    key: 'today:backendErrors.nextActionTooLong',
    required: ['max'],
    paramTypes: { max: 'number' },
  },
  ...
  'category.name_invalid': {
    key: 'settings:categories.backendErrors.nameInvalid',
    required: ['min', 'max'],
    paramTypes: { min: 'number', max: 'number' },
  },
  'category.color_invalid': {
    key: 'settings:categories.backendErrors.colorInvalid',
  },
  'category.reorder_invalid': {
    key: 'settings:categories.backendErrors.reorderInvalid',
  },
  'history.pagination_invalid': {
    key: 'history:backendErrors.paginationInvalid',
  },
  'day_theme.metadata_invalid': {
    key: 'theme:backendErrors.dayThemeMetadataInvalid',
  },
  'theme.invalid': { key: 'theme:backendErrors.invalid' },
  'theme.too_large': {
    key: 'theme:backendErrors.tooLarge',
    required: ['maxKiB'],
    paramTypes: { maxKiB: 'number' },
  },
  ...
  'backup.file_too_large': {
    key: 'backup:backendErrors.fileTooLarge',
    required: ['maxMiB'],
    paramTypes: { maxMiB: 'number' },
  },
  ...
  'backup.destination_invalid': {
    key: 'backup:backendErrors.destinationInvalid',
  },
  ...
  'backup.receipt_write_failed': {
    key: 'backup:backendErrors.receiptWriteFailed',
  },
...
function hasRequiredParams(
  ...
  if (!(
    definition.required?.every((name) => Object.hasOwn(params, name)) ?? true
  ))
    return false;
  return Object.entries(definition.paramTypes ?? {}).every(
    ([name, expected]) => typeof params[name] === expected,
  );
}
...
export function localizeAppWarning(
  value: unknown,
  translate: ErrorTranslator,
): string {
