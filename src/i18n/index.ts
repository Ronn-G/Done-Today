...
export function safeMissingTranslation(locale: unknown) {
  ...
  return (
    resources[resolved]?.errors?.messages?.unknown ??
    resources.en?.errors?.messages?.unknown ??
    emergencyMissingTranslation
  );
}

export const resolveDiagnosticLocale = (
  resolvedLanguage: unknown,
  language: unknown,
) => normalizeLocale(resolvedLanguage ?? language) ?? resourceFallbackLocale;

export function createI18nController(
  runtime: I18nRuntime,
  setup: () => void,
  reportMissing: (message: string) => void = (message) => console.warn(message),
) {
  ...
  const options: InitOptions = {
    resources,
    fallbackLng: resourceFallbackLocale,
    supportedLngs: ['vi', 'en'],
    ns: [...namespaces],
    defaultNS: 'common',
    ...
    parseMissingKeyHandler: () =>
      safeMissingTranslation(runtime.resolvedLanguage ?? runtime.language),
    missingKeyHandler: (_languages, namespace, key) => {
      ...
      if (!reportedMissingKeys.has(identity)) {
        ...
        const locale = resolveDiagnosticLocale(
          runtime.resolvedLanguage,
          runtime.language,
        );
        reportMissing(
          `Missing translation [locale=${locale}] [namespace=${namespace}] [key=${key}]`,
        );
      }
    },
  };
  const initialize = (locale: AppLocale): Promise<i18n> => {
    ...
    if (runtime.isInitialized) {
      const ready =
        runtime.language === locale
          ? Promise.resolve(runtime as i18n)
          : runtime.changeLanguage(locale).then(() => runtime as i18n);
...
const controller = createI18nController(i18next, () => {
...
export const initializeI18n = async (locale: AppLocale) => {
  ...
  if (!integerFormatterInstalled) {
    instance.services.formatter?.add('integer', (value, language) => {
      ...
      return formatCount(
        value,
        normalizeLocale(language) ?? resourceFallbackLocale,
      ).value;
    });
