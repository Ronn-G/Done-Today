...
describe('backend error and warning presentation contract', () => {
  ...
  it.each(['vi', 'en'] as const)('maps every stable code to a localized %s message', async locale => {
  ...
  it('re-renders the same structured payload in the active locale', async () => {
    ...
    const vietnamese = localizeAppError(
      payload,
      (key, options) => i18next.getFixedT('vi', 'errors')(key, options),
    );
    await i18next.changeLanguage('en');
    const english = localizeAppError(
      payload,
      (key, options) => i18next.getFixedT('en', 'errors')(key, options),
    );
  ...
  it('uses the localized generic fallback for unknown, malformed, or incomplete payloads', async () => {
    ...
    const present = (value: unknown) => localizeAppError(value, (key, options) => translate(key, options));
    expect(present({code: 'future.error', params: {}, message: 'SELECT secret'}))
      .toBe('Something went wrong. Please try again.');
    expect(present({code: 'work_item.task_too_long', params: {}}))
      .toBe('Something went wrong. Please try again.');
    expect(present({code: 'work_item.task_too_long', params: {max: '500'}}))
      .toBe('Something went wrong. Please try again.');
    expect(present(new Error(String.raw`C:\secret\done-today.sqlite`)))
      .toBe('Something went wrong. Please try again.');
  });
});
