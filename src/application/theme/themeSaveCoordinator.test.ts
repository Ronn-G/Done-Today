...
describe('ThemeSaveCoordinator', () => {
  it('persists the latest draft after debounce', async () => {
    vi.useFakeTimers();
    const saved: number[] = [];
    const coordinator = new ThemeSaveCoordinator<number>(
      async (value) => {
        saved.push(value);
      },
      () => undefined,
  ...
  it('does not let a stale response replace latest state', async () => {
  ...
  it('retains failed draft for retry', async () => {
    let attempts = 0;
    const coordinator = new ThemeSaveCoordinator<number>(
      async () => {
        attempts++;
