...
export class BackupService {
  ...
  private readonly flushTheme: () => Promise<void>;
  ...
  constructor(
    ...
    this.flushTheme = flushTheme;
    this.invalidate = invalidate;
  }
  private async flush() {
    await Promise.all([flushPendingJournalSaves(), this.flushTheme()]);
  }
  async export(presentation: BackupDialogPresentation) {
    await this.flush();
    const path = await this.repository.chooseExportPath(presentation);
    if (!path) return null;
    return this.repository.export(path);
  }
  async chooseAndPreview(presentation: BackupDialogPresentation) {
    await this.flush();
    const path = await this.repository.chooseImportPath(presentation);
    if (!path) return null;
  ...
  async import(
    ...
    await this.flush();
    const result = await this.repository.import(
      path,
      mode,
      applyTheme,
      confirmReimport,
