...
const exportPresentation={title:'Export Done Today backup',filterName:'Done Today backup'};
const importPresentation={title:'Restore from backup',filterName:'Done Today backup'};
describe('BackupService',()=>{
  it('forwards localized export presentation and treats cancellation as a no-op',async()=>{
    ...
    expect((repository as {chooseExportPath:ReturnType<typeof vi.fn>}).chooseExportPath).toHaveBeenCalledWith(exportPresentation);
    expect((repository as {export:ReturnType<typeof vi.fn>}).export).not.toHaveBeenCalled();
  });
  it('forwards localized import presentation and treats cancellation as a no-op',async()=>{
    const repository={chooseImportPath:vi.fn().mockResolvedValue(null),preview:vi.fn()} as never;
    const service=new BackupService(repository,vi.fn(),vi.fn());
    await expect(service.chooseAndPreview(importPresentation)).resolves.toBeNull();
    expect((repository as {chooseImportPath:ReturnType<typeof vi.fn>}).chooseImportPath).toHaveBeenCalledWith(importPresentation);
    expect((repository as {preview:ReturnType<typeof vi.fn>}).preview).not.toHaveBeenCalled();
  });
  ...
  it('does not change the active locale after import',async()=>{
    ...
    const repository={import:vi.fn().mockResolvedValue({mode:'replace',counts:{dailyLogs:1,workItems:1,workCategories:0,theme:false},remapped:0})}as never;
    const service=new BackupService(repository,vi.fn(),vi.fn());
