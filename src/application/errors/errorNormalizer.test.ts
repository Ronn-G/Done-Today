...
describe('structured Tauri error normalization', () => {
  ...
  it('accepts JSON-shaped rejections while rejecting raw and malformed values', () => {
    ...
    expect(normalizeAppError({ code: 'future.code', params: {} })).toEqual({
      kind: 'unknown',
    });
    expect(
      normalizeAppError(
        String.raw`C:\private\done-today.sqlite SELECT * FROM app_settings`,
      ),
    ).toEqual({ kind: 'unknown' });
  });
  ...
  it('normalizes known warnings and safely degrades unknown warnings', () => {
    ...
    expect(
      normalizeAppWarning({ code: 'backup.warning.future', params: {} }),
    ).toEqual({ kind: 'unknown' });
  });
});
