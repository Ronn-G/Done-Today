...
export class LocaleSwitchCoordinator{
  ...
  constructor(activate:(locale:AppLocale)=>Promise<void>,save:(locale:AppLocale)=>Promise<void>,state:(state:LocaleSwitchState)=>void){
    this.activate=activate;this.save=save;this.state=state;
  }
  attach(){this.attached=true}
  detach(){this.attached=false}
  private emit(state:LocaleSwitchState){if(this.attached)this.state(state)}
  switchTo(locale:AppLocale):Promise<void>{
    const revision=++this.revision;this.desiredLocale=locale;this.emit('saving');
    ...
    const persistence=this.persistenceQueue.then(async()=>{await activation;await this.save(locale)});
  ...
  retry(){return this.desiredLocale===null?Promise.resolve():this.switchTo(this.desiredLocale)}
  get desired(){return this.desiredLocale}
}
