...
describe('SaveCoordinator', () => {
  ...
  it('serializes writes, coalesces the queued revision and only marks the newest revision saved', async () => {
    ...
    coordinator.schedule('b');
    const queued = coordinator.flush();
    coordinator.schedule('c');
    const coalesced = coordinator.flush();
    expect(payloads).toEqual(['a']);
    expect(maximumActive).toBe(1);
    resolvers[0]();
    await vi.waitFor(() => expect(payloads).toEqual(['a', 'c']));
    expect(maximumActive).toBe(1);
    resolvers[1]();
    await Promise.all([first, queued, coalesced]);
  ...
  it('keeps a failed draft available and retries the same repository payload', async () => {
    ...
    const coordinator = new SaveCoordinator<string>(
      async (value) => {
        payloads.push(value);
        if (attempts++ === 0) throw new Error('work_item.update_failed');
        return value;
      },
      onPersisted,
      (state) => states.push(state),
    );
    coordinator.schedule('draft mới nhất');
    await expect(coordinator.flush()).rejects.toThrow(
      'work_item.update_failed',
    );
    expect(states).toEqual(['saving', 'error']);
    expect(payloads).toEqual(['draft mới nhất']);
    expect(onPersisted).not.toHaveBeenCalled();
    await coordinator.flush();
    expect(payloads).toEqual(['draft mới nhất', 'draft mới nhất']);
    expect(states.at(-1)).toBe('saved');
  ...
  it('retries a newer draft when the user continues typing after a failure', async () => {
    ...
    const coordinator = new SaveCoordinator(
      async (value: string) => {
        payloads.push(value);
        if (attempts++ === 0) throw new Error('fail');
        return value;
      },
      () => undefined,
      () => undefined,
    );
    coordinator.schedule('old draft');
    await expect(coordinator.flush()).rejects.toThrow('fail');
    coordinator.schedule('new draft');
    await coordinator.flush();
    expect(payloads).toEqual(['old draft', 'new draft']);
    coordinator.cancel();
  });
  it('keeps the current saved state without an additional timeout transition', async () => {
    ...
    coordinator.schedule('new revision');
