...
export function BootstrapFailure({retry}:{retry:()=>Promise<void>}){
  return <main role="alert"><p>Unable to start the application. Please try again.</p>
    <button type="button" onClick={()=>void retry()}>Retry</button>
  </main>;
}

export function createApplicationBootstrap(root:RootLike,dependencies:BootstrapDependencies){
