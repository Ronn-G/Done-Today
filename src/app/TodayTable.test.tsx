...
const renderTableChrome = (
  ...
  renderToStaticMarkup(
    <>
      ...
      <AddRowFooter categories={categories} onAddItem={onAddItem} />
    </>,
  );
...
describe('I18N-2 Today table and add-row flow', () => {
  it('renders Vietnamese table, category, empty and add-row copy', async () => {
    ...
    expect(html).toContain(
      '<caption class="sr-only">Bảng công việc trong ngày</caption>',
    );
  ...
  it('switches the same view data to English without mutating or writing user category data', async () => {
    ...
    await initializeI18n('vi');
    const vietnamese = renderTableChrome(
      group,
      [category],
      onAddItem,
      onToggle,
    ...
    const english = renderTableChrome(group, [category], onAddItem, onToggle);
  ...
  it('localizes only the virtual null-group label and preserves its null identity', async () => {
    await initializeI18n('vi');
    const vietnamese = renderTableChrome(virtualGroup, []);
    await initializeI18n('en');
    const english = renderTableChrome(virtualGroup, []);
    ...
    expect(virtualGroup).toMatchObject({
      id: null,
      category: null,
      name: null,
    });
  ...
  it('uses the table-header semantic variable only for the Today column-header surface', async () => {
    ...
    const categoryHeaderRule = styles.match(
      /\.category-row th\s*\{([^}]*)\}/,
    )?.[1];
