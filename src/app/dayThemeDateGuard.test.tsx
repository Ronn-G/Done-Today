...
function DateGuardProbe({date, onRead}: {date: string; onRead: (value: string | null) => void}) {
  const activeDate = useActiveDayThemeDate(date);
  return <button type="button" onClick={() => onRead(activeDate.current)}>Read active date</button>;
}

describe('Day Theme date guard', () => {
  it('restores the active date after the development StrictMode effect replay', () => {
    const onRead = vi.fn();
    render(<StrictMode><DateGuardProbe date="2026-07-29" onRead={onRead}/></StrictMode>);
