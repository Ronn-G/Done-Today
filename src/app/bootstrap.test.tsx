...
describe('application bootstrap', () => {
  it('renders the application once after a persisted locale initializes', async () => {
    ...
    const initializeI18n = vi.fn(async () => undefined);
    const bootstrap = createApplicationBootstrap(
      { render: (node) => rendered.push(node) },
      {
        localeService: {
          initialize: async () => ({ locale: 'en', source: 'persisted' }),
        },
        initializeI18n,
    ...
    await bootstrap.run();
    expect(initializeI18n).toHaveBeenCalledWith('en');
  ...
  it('renders a recoverable fallback and retries a transient initialization failure', async () => {
    ...
    expect(isValidElement(fallback) && fallback.type).toBe(BootstrapFailure);
    if (!isValidElement<{ retry: () => Promise<void> }>(fallback))
      throw new Error('Expected fallback element');
    await fallback.props.retry();
    expect(rendered).toHaveLength(2);
    expect(rendered[1]).toBe('application');
  ...
  it('keeps repeated failure recoverable without an automatic retry loop or raw key', async () => {
    ...
    if (!isValidElement<{ retry: () => Promise<void> }>(first))
      throw new Error('Expected fallback element');
  ...
  it('coalesces concurrent bootstrap attempts', async () => {
    ...
    const bootstrap = createApplicationBootstrap(
      { render: (node) => rendered.push(node) },
      {
        ...
        initializeI18n: async () => waiting,
        renderApplication: () => 'application',
        reportError: vi.fn(),
      },
    );
    const first = bootstrap.run();
    const second = bootstrap.run();
