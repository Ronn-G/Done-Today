...
import {
  ...
  SaveCoordinator,
  type SaveState,
} from '../application/journal/saveCoordinator';
import {
  ThemeSaveCoordinator,
  type ThemeSaveState,
} from '../application/theme/themeSaveCoordinator';
...
import {
  ...
  initialFloatingThemePanelState,
} from '../features/settings/FloatingThemeCustomizer';
...
import {
  getRowActionDestinations,
  moveItemAfterFlush,
  positionRowActionMenu,
} from '../features/daily-log/rowActionMenu';
import {
  compatibilityLocale,
  normalizeLocale,
...
export function resolveDayThemeForLog(
  log: Pick<DailyLog, 'themeId' | 'themeVersion'> | null,
) {
  return dayThemeRegistry.resolve(
    log?.themeId ?? null,
    log?.themeVersion ?? null,
  );
}
export function shouldPersistDayTheme(
  log: DailyLog | null,
  metadata: DayThemeMetadata,
) {
  return (
    log !== null || Object.values(metadata).some((value) => value !== null)
  );
}
export function mergeSavedDayTheme(
...
export function shouldAcceptDayThemeCompletion(
...
export function App() {
  ...
  const [themeSaveError, setThemeSaveError] =
    useState<NormalizedAppError | null>(null);
  const [floatingThemePanel, setFloatingThemePanel] = useState(
    initialFloatingThemePanelState,
  );
  ...
  const themeCoordinator = useMemo(
    () =>
      new ThemeSaveCoordinator<ThemePreferences>(
        (value) => themeRepository.save(value),
        setThemeSaveState,
      ),
    [],
  );
  ...
  useEffect(() => {
    ...
    apply();
    localStorage.setItem('done-today-theme', theme);
    media.addEventListener('change', apply);
    return () => media.removeEventListener('change', apply);
  }, [theme]);
  const activePalette = resolvePalette(theme, systemDark);
  useEffect(() => {
    document.documentElement.classList.toggle('dark', activePalette === 'dark');
    applyThemePreferences(themePreferences, activePalette);
  }, [activePalette, themePreferences]);
  useEffect(() => {
    void themeRepository
      .load()
      .then((saved) => {
        if (saved) setThemePreferences(saved);
      })
      .catch(() => setThemePreferences(defaultThemePreferences()));
  }, []);
  useEffect(
    () => () => {
      void themeCoordinator.flush().catch(() => undefined);
      themeCoordinator.cancel();
    },
    [themeCoordinator],
  );
  useEffect(() => {
    const flush = () => void themeCoordinator.flush().catch(() => undefined);
  ...
  const commitTheme = useCallback(
    (next: ThemePreferences) => {
      ...
      themeCoordinator.schedule(next);
    },
    [themeCoordinator],
  );
  const flushTheme = useCallback(async () => {
    try {
      await themeCoordinator.flush();
      setThemeSaveError(null);
    } catch (reason) {
  ...
  const resetTheme = useCallback(() => {
    if (confirm(tTheme('actions.resetConfirmation')))
      commitTheme(defaultThemePreferences());
  }, [commitTheme, tTheme]);
  const themeController: ThemeCustomizerController = {
    mode: theme,
    setMode: setTheme,
    preferences: themePreferences,
    setPreferences: setThemePreferences,
    activePalette,
  ...
  const openThemePanel = () =>
    setFloatingThemePanel((current) => {
      const next = { ...current, open: true };
      localStorage.setItem(
        'done-today-floating-theme-panel',
        JSON.stringify(next),
      );
      return next;
    });
  return (
    <div className="app-shell">
      ...
      <main>
        {route.page === 'day' && (
          <DayEditor
            key={`${route.date}-${dataRevision}`}
            date={route.date}
            onOpenTheme={openThemePanel}
          />
        )}
          ...
          <SettingsPage
            ...
            onImported={() => {
              ...
              void themeRepository
                .load()
                .then((value) =>
                  setThemePreferences(value ?? defaultThemePreferences()),
                );
              navigate({ page: 'day', date: today() });
            }}
          />
        )}
      </main>
      {route.page === 'day' && (
        <FloatingThemeCustomizer
          controller={themeController}
          state={floatingThemePanel}
          setState={setFloatingThemePanel}
        />
      )}
    </div>
  );
...
function DayEditor({
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [nextLog, nextCategories, nextStreak] = await Promise.all([
        service.getDailyLog(date),
        service.listCategories(true),
        service.getCurrentStreak(today()),
      ]);
      setLog(nextLog);
      ...
      setCollapsed(
        parseCollapsedCategoryState(
          localStorage.getItem('done-today-collapsed-categories'),
  ...
  const groups = useMemo(
    () => groupDailyItems(items, categories),
    [items, categories],
  );
  ...
  const resolvedDayTheme = useMemo(
    () =>
      dayThemeRegistry.resolve(
        activeDayTheme.themeId,
        activeDayTheme.themeVersion,
      ),
    [activeDayTheme],
  );
  const openDayThemePicker = () => {
    ...
    setDayThemePickerOpen(true);
  };
  const closeDayThemePicker = () => {
    setPreviewDayTheme(null);
    setDayThemePickerOpen(false);
  };
  ...
  const remove = async (item: WorkItem) => {
    try {
      await service.deleteWorkItem(item.id);
      setLog((previous) =>
        previous
          ? {
              ...previous,
    ...
    if (item.task.trim())
      void refreshStreak().catch((reason) =>
        setError(normalizeAppError(reason)),
      );
  };
  const move = async (bucket: WorkItem[], index: number, direction: -1 | 1) => {
    ...
    try {
      const saved = await service.reorderWorkItems(
        log.id,
        ...
        current
          ? {
              ...
              items: current.items.map(
                (item) => saved.find((value) => value.id === item.id) ?? item,
              ),
            }
          : current,
      );
    } catch (reason) {
      const failure = normalizeAppError(reason);
      void load().finally(() => setError(failure));
    }
  };
  const toggleGroup = (id: string | null) => {
    ...
    setCollapsed((current) => {
      const next = current.includes(key)
        ...
        : [...current, key];
      localStorage.setItem(
        'done-today-collapsed-categories',
        JSON.stringify({ schemaVersion: 1, collapsedCategoryIds: next }),
      );
      return next;
    });
  };
  const changeCategory = async (item: WorkItem, categoryId: string | null) => {
    try {
      const saved = await service.moveWorkItemToCategory(item.id, categoryId);
      updateLocal(saved);
    } catch (reason) {
      setError(normalizeAppError(reason));
      throw reason;
      ...
      <div className="content">
        ...
        {error && (
          <div className="page-error" role="alert">
            {localizeAppError(error, toErrorTranslator(tErrors))}
        ...
        <section className="table-card">
          {loading ? (
            <div className="message">
              <LoaderCircle className="spin" size={20} /> {t('status.loading')}
            </div>
          ) : (
                    ...
                    return [
                      ...
                      ...(hidden
                        ...
                        : group.items.map((item, index) => {
                            const bucket = group.items.filter(
                              (value) =>
                            ...
                            return (
                              <WorkRow
                                ...
                                onJournalActivityChanged={refreshStreak}
                                onCategoryChange={(categoryId) =>
                                  changeCategory(item, categoryId)
                                }
                                onDelete={() => void remove(item)}
                                onMoveUp={() =>
                                  void move(bucket, bucketIndex, -1)
                                }
                                onMoveDown={() =>
                                  void move(bucket, bucketIndex, 1)
                                }
                                canMoveUp={bucketIndex > 0}
                                canMoveDown={bucketIndex < bucket.length - 1}
                              />
                            );
                          })),
                    ];
                  })}
                  {!items.length && <TodayEmptyState />}
                </tbody>
              </table>
            </div>
          )}
        ...
        <Suspense fallback={null}>
          <LazyDayThemePicker
            ...
            onApplied={() => {
              setPreviewDayTheme(null);
              setDayThemePickerOpen(false);
              setDayThemeFeedback(tTheme('dayTheme.picker.saved'));
            }}
  ...
  return (
    <>
      <DayCover
        ...
        actions={
          <div className="date-nav">
            <button
              aria-label={t('dateControls.previous')}
              title={t('dateControls.previous')}
              onClick={() => onGo(addLocalDays(date, -1))}
            >
              <ChevronLeft size={18} />
            </button>
            <label className="date-picker">
              ...
              <input
                aria-label={t('dateControls.choose')}
                type="date"
                value={date}
                onChange={(event) =>
                  isValidLocalDate(event.target.value) &&
                  onGo(event.target.value)
                }
              />
            </label>
            <button
              aria-label={t('dateControls.next')}
              title={t('dateControls.next')}
              onClick={() => onGo(addLocalDays(date, 1))}
            >
              <ChevronRight size={18} />
            </button>
            {!current && (
              <button className="today-button" onClick={() => onGo(today())}>
                {t('dateControls.today')}
              </button>
            )}
            <button className="day-theme-trigger" onClick={onOpenDayTheme}>
      ...
      <section className="stats" aria-label={t('stats.label')}>
        <Stat
          label={t('stats.total')}
          value={formatCount(stats.total, locale).value}
        />
        <Stat
          label={t('stats.completed')}
          value={formatCount(stats.completed, locale).value}
        />
        <div className="stat progress-stat">
          <span>{t('stats.completionRate')}</span>
          <strong>{formatPercent(stats.percentage / 100, locale)}</strong>
          <div
            className="progress"
            role="progressbar"
            aria-label={t('stats.completionRate')}
        ...
        <Stat
          ...
          value={t('stats.streakValue', { count: currentStreak })}
        />
      </section>
    </>
  );
...
export function TodayTableHeader() {
  ...
  return (
    <>
      ...
      <thead>
        <tr>
          <th className="order-col">{t('table.columns.order')}</th>
          <th>{t('table.columns.task')}</th>
          <th>{t('table.columns.result')}</th>
          <th>{t('table.columns.nextAction')}</th>
...
export function TodayCategoryHeader({
  group,
  hidden,
  onAddItem,
  onToggle,
  ...
  group: CategoryGroup;
  hidden: boolean;
  onAddItem: (categoryId: string | null) => void | Promise<void>;
    ...
    <tr className="category-row">
      <th colSpan={6}>
        <div className="category-header">
          ...
          <h2>
            {displayName}
            {!group.isActive && <small> · {t('categories.hidden')}</small>}
          </h2>
          <span>
            {t('categories.completedCount', {
              completed: group.completedItems,
              count: group.totalItems,
            })}
          </span>
          <button
            aria-label={t('categories.addItem', { category: displayName })}
            onClick={() => void onAddItem(group.id)}
          >
            +
          </button>
          <button
            aria-label={
              hidden
                ? t('categories.expand', { category: displayName })
                : t('categories.collapse', { category: displayName })
            }
...
function resolveAddRowCategorySelection(
  value: string,
...
export function AddRowFooter({
  ...
  return (
    <footer className="table-footer">
      <label className="add-row-select">
        <span>{t('addItem.label')}</span>
        <select
          aria-label={t('addItem.accessibility.chooseCategory')}
          onChange={(event) => {
            submitAddRowCategorySelection(event.target.value, onAddItem);
            event.target.value = '';
          }}
          defaultValue=""
        >
...
export function submitWorkStatusSelection(
  value: string,
...
export function journalEligibilityChanged(
  previousTask: string,
  nextTask: string,
) {
...
export function WorkRow({
  ...
  onCategoryChange: (id: string | null) => Promise<void>;
  onDelete: () => void;
  onJournalActivityChanged: () => void | Promise<void>;
  onMoveUp: () => void;
  onMoveDown: () => void;
  canMoveUp: boolean;
  canMoveDown: boolean;
  ...
  const trackEligibility = useMemo(
    () => createJournalEligibilityTracker(initialPersistedTask),
  ...
  const coordinator = useMemo(() => {
    return new SaveCoordinator<UpdateWorkItem, WorkItem>(
      ...
      (saved, draft) => {
        ...
        const eligibilityChanged = trackEligibility(saved.task);
        onChange(reconciled);
  ...
  const compositionEnd = (
    ...
    changeText(field, event.currentTarget.value);
  };
  ...
  return (
    <tr className="editable-row" data-group-index={dataIndex}>
      ...
      <td>
        <textarea
          ...
          aria-label={t('fields.task.label')}
          ...
          autoFocus={autoFocus}
          onFocus={onFocused}
          onChange={(e) => changeText('task', e.target.value)}
          onCompositionStart={compositionStart}
          onCompositionEnd={(e) => compositionEnd('task', e)}
          onBlur={flush}
          onKeyDown={escape}
        />
      </td>
      <td>
        <textarea
          className="work-item-editor result-editor"
          aria-label={t('fields.result.label')}
          placeholder={t('fields.result.placeholder')}
          value={item.result}
          maxLength={2000}
          rows={2}
          onChange={(e) => changeText('result', e.target.value)}
          onCompositionStart={compositionStart}
          onCompositionEnd={(e) => compositionEnd('result', e)}
          onBlur={flush}
          onKeyDown={escape}
        />
      </td>
      <td>
        <textarea
          className="work-item-editor next-action-editor"
          aria-label={t('fields.nextAction.label')}
          placeholder={t('fields.nextAction.placeholder')}
          value={item.nextAction}
          maxLength={1000}
          rows={2}
          onChange={(e) => changeText('nextAction', e.target.value)}
          onCompositionStart={compositionStart}
          onCompositionEnd={(e) => compositionEnd('nextAction', e)}
          onBlur={flush}
          onKeyDown={escape}
        />
      </td>
      <td>
        <select
          aria-label={t('fields.status.label')}
          className={`status-select ${item.status}`}
          value={item.status}
          onChange={(e) =>
            submitWorkStatusSelection(e.target.value, (status) =>
              schedule({ ...latest.current, status }, true),
            )
          }
        >
        ...
        <SaveIndicator state={state} retry={flush} />
      </td>
      <td className="row-actions">
        <RowActionMenu
          item={item}
          categories={categories}
          flush={flushBeforeAction}
          onMove={onCategoryChange}
          onDelete={onDelete}
        />
      </td>
    </tr>
...
export function requiresDeleteConfirmation(
  item: Pick<WorkItem, 'task' | 'result' | 'nextAction'>,
) {
  return Boolean(
    item.task.trim() || item.result.trim() || item.nextAction.trim(),
  );
}
export function completeDeleteConfirmation(
  confirmed: boolean,
  onDelete: () => void,
) {
  ...
  return (
    ...
    >
      <div className="delete-confirmation-content">
        <h2 id={`${id}-title`}>{t('item.confirmDelete.title')}</h2>
        <p id={`${id}-body`}>{t('item.confirmDelete.body')}</p>
...
export function RowActionMenuContent({
  item,
  categories,
  position,
  onMove,
  ...
  const { t } = useTranslation('today');
  const label = item.task.trim() || t('item.untitled');
  const actionsLabel = t('item.accessibility.actionsForTask', { task: label });
  return (
    <div
      ref={menuRef}
      ...
      style={position}
    >
      <strong>{t('categories.moveTo')}</strong>
      {getRowActionDestinations(categories).map((category) => (
        <button
          role="menuitem"
          key={category.id}
          onClick={() => onMove(category.id)}
        >
          {item.categoryId === category.id ? <Check size={15} /> : <span />}
          {category.name}
        </button>
      ))}
      <button role="menuitem" onClick={() => onMove(null)}>
        {item.categoryId === null ? <Check size={15} /> : <span />}
        {t('categories.other')}
      </button>
      <hr />
      <button className="danger" role="menuitem" onClick={onDelete}>
...
export function RowActionMenu({
  ...
  const toggle = () => {
    if (!open && trigger.current) {
      setPosition(
        positionRowActionMenu(trigger.current.getBoundingClientRect(), {
          width: innerWidth,
          height: innerHeight,
        }),
      );
  ...
  const move = async (categoryId: string | null) => {
    try {
      await moveItemAfterFlush(categoryId, flush, onMove);
      close();
    } catch {
      return;
    }
  ...
  const requestDelete = () => {
    setOpen(false);
    if (requiresDeleteConfirmation(item)) setConfirmingDelete(true);
    else onDelete();
  };
  const finishDelete = (confirmed: boolean) => {
    setConfirmingDelete(false);
    if (!completeDeleteConfirmation(confirmed, onDelete))
      requestAnimationFrame(() => trigger.current?.focus());
  };
  const label = item.task.trim() || t('item.untitled');
  const actionsLabel = t('item.accessibility.actionsForTask', { task: label });
      ...
      {open &&
        createPortal(
          <RowActionMenuContent
            ...
            categories={categories}
            position={position}
            onMove={(categoryId) => void move(categoryId)}
...
export function mergeHistorySummaries(
  previous: DailyLogSummary[],
  next: DailyLogSummary[],
  append: boolean,
) {
  return append
    ? [
        ...previous.filter(
          (existing) => !next.some((entry) => entry.id === existing.id),
        ),
        ...next,
      ]
    : next;
...
function HistoryPage() {
  ...
  const load = useCallback(async (targetPage: number, append: boolean) => {
    ...
    try {
      ...
      setItems((previous) =>
        mergeHistorySummaries(previous, result.items, append),
      );
      setPage(result.page);
...
export function HistoryView({
  ...
  loading: boolean;
  loadingMore: boolean;
  error: boolean | NormalizedAppError | null;
  hasMore: boolean;
  onRetry: () => void;
  onLoadMore: () => void;
    ...
    <div className="content">
      ...
      {loading ? (
        <div
          ...
          aria-atomic="true"
        >
          <LoaderCircle className="spin" /> {t('status.loading')}
        </div>
      ) : error ? (
        <div className="page-error" role="alert">
          {typeof error === 'object'
        ...
        <section className="history-list" aria-label={t('accessibility.list')}>
          {items.map((summary) => {
            ...
            return (
              <button
                ...
                onClick={() => onOpenDay(summary.logDate)}
              >
                <div className="history-date">
                  <strong>
                    {formatShortLocalDate(summary.logDate, locale)}
                  </strong>
                  <span>{formatWeekdayLocalDate(summary.logDate, locale)}</span>
                </div>
                  ...
                  {summary.previewTasks.length > 0 && (
                    <ul>
                      {summary.previewTasks.map((task, index) => (
                        <li key={`${summary.id}-${index}`}>{task}</li>
                      ))}
                    </ul>
                  )}
                </div>
                <div
                  ...
                  role="progressbar"
                  aria-label={t('accessibility.completionRateForDay', { date })}
          ...
          {hasMore && (
            <button
              ...
              onClick={onLoadMore}
            >
              {loadingMore ? t('status.loadingMore') : t('actions.loadMore')}
            </button>
          )}
        </section>
      )}
...
export function SettingsPageView({ children }: { children: React.ReactNode }) {
  ...
  return (
    ...
    >
      <header>
        <p className="eyebrow">{t('heading.eyebrow')}</p>
        <h1 id="settings-page-title">{t('heading.title')}</h1>
        <p className="subtitle">{t('heading.subtitle')}</p>
      </header>
      {children}
    </div>
...
function SettingsPage({
  ...
  controller: ThemeCustomizerController;
  onImported: () => void;
}) {
  return (
    <SettingsPageView>
      <LanguageSettings />
      <BackupSettings flushTheme={controller.flush} onImported={onImported} />
      <CategorySettings service={service} />
