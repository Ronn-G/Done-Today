...
export const backupDailyLogSchema = z
  .object({
    id,
    logDate: date,
    createdAt: timestamp,
...
export const backupWorkItemSchema = z
  .object({
    id,
    dailyLogId: id,
...
export const backupPayloadSchema = z
  .object({
    ...
    workItems: z.array(backupWorkItemSchema),
    workCategories: z.array(backupCategorySchema),
  ...
  .superRefine((payload, context) => {
    ...
    unique(
      payload.dailyLogs.map((value) => value.id),
      'dailyLogs',
    );
    unique(
      payload.workItems.map((value) => value.id),
      'workItems',
    );
    unique(
      payload.workCategories.map((value) => value.id),
      'workCategories',
    );
    const dates = new Set<string>();
    payload.dailyLogs.forEach((value, index) => {
      if (dates.has(value.logDate))
        context.addIssue({
          code: 'custom',
          path: ['dailyLogs', index, 'logDate'],
          message: 'Ngày bị trùng.',
        });
      dates.add(value.logDate);
    });
    ...
    payload.workItems.forEach((value, index) => {
      if (!logs.has(value.dailyLogId))
        context.addIssue({
          code: 'custom',
          path: ['workItems', index, 'dailyLogId'],
          message: 'Không tìm thấy ngày tham chiếu.',
        });
      if (value.categoryId !== null && !categories.has(value.categoryId))
        context.addIssue({
          code: 'custom',
          path: ['workItems', index, 'categoryId'],
          message: 'Không tìm thấy nhóm tham chiếu.',
        });
    });
  });
