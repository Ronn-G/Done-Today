...
describe('theme colors',()=>{
  ...
  it('rejects unsafe CSS',()=>{for(const value of ['red','var(--x)','url(x)','#12','rgba(0,0,0,1)'])expect(isValidHexColor(value)).toBe(false)});
  it('normalizes short HEX',()=>expect(normalizeHexColor('#a3F')).toBe('#AA33FF'));
  ...
  it('keeps default sidebar and stats text readable',()=>{for(const colors of [defaultThemePreferences().lightColors,defaultThemePreferences().darkColors]){expect(calculateContrastRatio(colors.sidebarText,colors.sidebarBackground)).toBeGreaterThanOrEqual(4.5);expect(calculateContrastRatio(colors.sidebarActiveText,colors.sidebarActiveBackground)).toBeGreaterThanOrEqual(4.5);expect(calculateContrastRatio(colors.statsPanelPrimaryText,colors.statsPanelBackground)).toBeGreaterThanOrEqual(4.5);expect(calculateContrastRatio(colors.statsPanelSecondaryText,colors.statsPanelBackground)).toBeGreaterThanOrEqual(4.5)}});
  it('upgrades schema v1 while preserving existing colors',()=>{const current=defaultThemePreferences();const removeStats=(colors:ThemeColors)=>{const legacy:Partial<ThemeColors>={...colors};for(const key of ['statsPanelBackground','statsPanelBorder','statsPanelPrimaryText','statsPanelSecondaryText','statsPanelProgressTrack','statsPanelProgressFill'] as const)delete legacy[key];return legacy};const upgraded=parseThemePreferences({...current,lightColors:removeStats(current.lightColors),darkColors:removeStats(current.darkColors),schemaVersion:1});expect(upgraded.schemaVersion).toBe(2);expect(upgraded.lightColors.accent).toBe(current.lightColors.accent);expect(upgraded.lightColors.statsPanelBackground).toBe(current.lightColors.cardBackground);expect(upgraded.lightColors.statsPanelProgressFill).toBe(current.lightColors.accent)});
  it('falls back safely when schema is corrupt',()=>{const parsed=themePreferencesSchema.safeParse({schemaVersion:0});expect(parsed.success?parsed.data:defaultThemePreferences()).toMatchObject({selectedPresetId:'done-today'})});
});
