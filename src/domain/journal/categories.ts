...
export type CategoryGroup={id:string|null;category:WorkCategory|null;name:string|null;color:string|null;isActive:boolean;items:WorkItem[];totalItems:number;completedItems:number};
...
export function groupDailyItems(items:readonly WorkItem[],categories:readonly WorkCategory[]):CategoryGroup[]{
  ...
  if(used.has(null)){const grouped=sortItemsByBucket(used.get(null)??[]);result.push({id:null,category:null,name:null,color:null,isActive:true,items:grouped,totalItems:grouped.length,completedItems:grouped.filter(item=>item.status==='completed').length})}
  return result;
}
