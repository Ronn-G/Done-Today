...
export const categoryColorSchema = z
  .string()
  .regex(/^#[0-9A-Fa-f]{6}$/, 'Màu nhóm phải dùng HEX #RRGGBB.');
...
export function groupDailyItems(
  ...
  for (const item of items) {
    const key =
      item.categoryId && byId.has(item.categoryId) ? item.categoryId : null;
    const current = used.get(key) ?? [];
    current.push(item);
    used.set(key, current);
  }
  const active = categories
    .filter((category) => category.isActive && used.has(category.id))
    .sort((a, b) => a.position - b.position);
  const inactive = categories
    .filter((category) => !category.isActive && used.has(category.id))
    .sort((a, b) => a.position - b.position);
  const make = (category: WorkCategory): CategoryGroup => {
    const grouped = sortItemsByBucket(used.get(category.id) ?? []);
    return {
      id: category.id,
      category,
      name: category.name,
      ...
      completedItems: grouped.filter((item) => item.status === 'completed')
        .length,
    };
  };
  const result = [...active.map(make), ...inactive.map(make)];
  if (used.has(null)) {
    const grouped = sortItemsByBucket(used.get(null) ?? []);
    result.push({
      id: null,
      category: null,
      name: null,
...
export const collapsedCategoryStateSchema = z.object({
  schemaVersion: z.literal(1),
  collapsedCategoryIds: z.array(z.string()).catch([]),
});
export type CollapsedCategoryState = z.infer<
  typeof collapsedCategoryStateSchema
>;
export function parseCollapsedCategoryState(
  raw: string | null,
  validIds: readonly string[],
): CollapsedCategoryState {
  try {
    const parsed = collapsedCategoryStateSchema.parse(JSON.parse(raw ?? ''));
    const valid = new Set(validIds);
    return {
      schemaVersion: 1,
      collapsedCategoryIds: parsed.collapsedCategoryIds.filter(
        (id) => id === '__other__' || valid.has(id),
      ),
    };
  } catch {
    return { schemaVersion: 1, collapsedCategoryIds: [] };
  }
}
