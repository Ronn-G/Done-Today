...
describe('current journal streak', () => {
  it('returns zero without activity', () =>
    expect(calculateCurrentStreak([], '2026-07-24')).toBe(0));
  it('counts today as one', () =>
    expect(calculateCurrentStreak(['2026-07-24'], '2026-07-24')).toBe(1));
  ...
  it('accepts unordered input without mutating it', () => {
    const dates = ['2026-07-24', '2026-07-22', '2026-07-23'];
    const before = [...dates];
    expect(calculateCurrentStreak(dates, '2026-07-24')).toBe(3);
  ...
  it('crosses a month boundary', () => {
    expect(
      calculateCurrentStreak(['2026-02-28', '2026-03-01'], '2026-03-01'),
    ).toBe(2);
  });
  it('crosses a year boundary', () => {
    expect(
      calculateCurrentStreak(['2025-12-31', '2026-01-01'], '2026-01-01'),
    ).toBe(2);
  });
  it('crosses leap day', () => {
    expect(
  ...
  it('ignores malformed activity dates and handles malformed today safely', () => {
    expect(
      calculateCurrentStreak(['bad', '2026-7-24', '2026-07-24'], '2026-07-24'),
    ).toBe(1);
