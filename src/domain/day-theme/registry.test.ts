...
describe('DayThemeRegistry', () => {
  ...
  it.each([
    ...
    [
      ...
      {
        tokens: {
          ...doneTodayDefaultDayTheme.tokens,
          accent: 'url(https://invalid)',
        },
      },
    ],
    [
      'gradient',
      {
        cover: { ...doneTodayDefaultDayTheme.cover, fallbackGradient: '#fff' },
      },
    ],
  ])('rejects an invalid %s', (_name, overrides) => {
    const registry = new DayThemeRegistry([]);
    expect(() => registry.register(definition(overrides))).toThrow(
      DayThemeValidationError,
    );
  });
  ...
  it('stores immutable copies and accepts missing optional assets', () => {
    ...
    const mutableCover = source.cover as {
      assetId?: string;
      motifAssetId?: string;
    };
  ...
  it('has name and description resources in Vietnamese and English', () => {
    for (const locale of ['vi', 'en'] as const) {
      ...
      for (const definition of dayThemeRegistry.list()) {
        ...
        expect(
          theme[definition.descriptionKey.replace('theme:', '')],
        ).toBeTruthy();
      }
    }
  });
