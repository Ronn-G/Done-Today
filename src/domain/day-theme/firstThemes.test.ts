...
import {dayThemeAssetIds, isKnownDayThemeAssetId, loadDayThemeAsset} from './assets';
...
describe('first Day Theme definitions', () => {
  it('defines exactly Sakura, Coffee and Rainy at version 1 as immutable adaptive themes', () => {
    expect(firstDayThemes.map(theme => [theme.id, theme.version, theme.mode])).toEqual([
      ['sakura', 1, 'adaptive'],
      ['coffee', 1, 'adaptive'],
      ['rainy', 1, 'adaptive'],
    ]);
  ...
  it('keeps body, muted text, focus and cover text contrast at accessible levels', () => {
    for (const theme of firstDayThemes) {
      for (const variant of [theme.variants?.light, theme.variants?.dark]) {
        ...
        if (!variant) continue;
        expect(contrast(variant.tokens.dayText, variant.tokens.daySurface)).toBeGreaterThanOrEqual(4.5);
        expect(contrast(variant.tokens.dayText, variant.tokens.pageBackground)).toBeGreaterThanOrEqual(4.5);
        expect(contrast(variant.tokens.dayTextMuted, variant.tokens.daySurface)).toBeGreaterThanOrEqual(4.5);
        expect(contrast(variant.tokens.focusRing, variant.tokens.daySurface)).toBeGreaterThanOrEqual(3);
        const coverText = variant.cover.textTone === 'light' ? '#ffffff' : variant.tokens.dayText;
  ...
  it('uses known logical motif IDs without paths or network URLs', async () => {
    ...
    for (const theme of firstDayThemes) {
      ...
      expect(JSON.stringify(theme)).not.toMatch(/https?:|[A-Za-z]:\\|(?:^|["'])\//);
    }
    await expect(loadDayThemeAsset('theme.missing.cover.motif')).resolves.toBeNull();
  });
  ...
  it('provides natural Vietnamese and English names and descriptions', () => {
    for (const locale of ['vi', 'en'] as const) {
      ...
      for (const theme of firstDayThemes) {
        expect(catalog[theme.nameKey.replace('theme:', '')]?.trim()).toBeTruthy();
        expect(catalog[theme.descriptionKey.replace('theme:', '')]?.trim()).toBeTruthy();
      }
    }
  });
});
