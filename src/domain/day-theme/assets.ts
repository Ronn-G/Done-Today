...
const assetLoaders: Record<DayThemeAssetId, () => Promise<string>> = {
  'theme.sakura.cover.motif': () => import('./assets/sakura-motif.svg?url').then(module => module.default),
  'theme.coffee.cover.motif': () => import('./assets/coffee-motif.svg?url').then(module => module.default),
  'theme.rainy.cover.motif': () => import('./assets/rainy-motif.svg?url').then(module => module.default),
};

export function isKnownDayThemeAssetId(assetId: string): assetId is DayThemeAssetId {
