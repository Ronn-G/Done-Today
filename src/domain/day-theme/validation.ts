...
const TRANSLATION_KEY_PATTERN =
  /^theme:[a-z][A-Za-z0-9]*(?:\.[a-z][A-Za-z0-9]*){2,}$/;
...
export function isValidDayThemeId(value: string): boolean {
  return (
    value.length > 0 &&
    value.length <= DAY_THEME_ID_MAX_LENGTH &&
...
function validGradient(value: string): boolean {
  const trimmed = value.trim();
  return (
    validCssValue(trimmed) && /^(?:linear|radial)-gradient\(/i.test(trimmed)
  );
}
...
export function validateDayThemeDefinition(
  value: DayThemeDefinition,
): readonly string[] {
  const issues: string[] = [];
  ...
  if (!TRANSLATION_KEY_PATTERN.test(value.descriptionKey))
    issues.push('descriptionKey');
  ...
  const validateTokens = (
    prefix: string,
    tokens: DayThemeDefinition['tokens'],
  ) => {
  ...
  const validateCover = (
    prefix: string,
    cover: DayThemeDefinition['cover'],
  ) => {
    if (!validGradient(cover.fallbackGradient))
      issues.push(`${prefix}.fallbackGradient`);
    if (!validCssValue(cover.overlay)) issues.push(`${prefix}.overlay`);
    if (!['light', 'dark'].includes(cover.textTone))
      issues.push(`${prefix}.textTone`);
  };
  validateTokens('tokens', value.tokens);
  validateCover('cover', value.cover);
  if (!isValidDayThemeAssetId(value.cover.assetId))
    issues.push('cover.assetId');
  if (!isValidDayThemeAssetId(value.cover.motifAssetId))
    issues.push('cover.motifAssetId');
  ...
  if (!validCssValue(value.calendar.indicatorColor))
    issues.push('calendar.indicatorColor');
  if (
    value.calendar.symbol !== undefined &&
    [...value.calendar.symbol].length > 4
  ) {
    issues.push('calendar.symbol');
  }
  if (
    value.typography?.journalFontRole !== undefined &&
    !['ui', 'journal', 'display'].includes(value.typography.journalFontRole)
  ) {
    issues.push('typography.journalFontRole');
  }
  if (
    value.typography?.headingWeight !== undefined &&
  ...
  if (
    !['calm', 'warm', 'nature', 'night', 'seasonal', 'minimal'].includes(
      value.metadata.category,
    )
  ) {
    issues.push('metadata.category');
  }
  if (typeof value.metadata.builtIn !== 'boolean')
    issues.push('metadata.builtIn');
